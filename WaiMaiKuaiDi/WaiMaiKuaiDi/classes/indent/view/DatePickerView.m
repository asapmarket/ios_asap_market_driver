...
- (void)show {
    ...
    self.month = [NSString stringWithFormat:@"%ld%@", [self.timeArr[1] integerValue],NSLocalizedString(kMon, nil)];
    self.day = [NSString stringWithFormat:@"%ld%@", [self.timeArr[2] integerValue],NSLocalizedString(kDay, nil)];
    self.hour = [NSString stringWithFormat:@"%ld%@", [self.timeArr[3] integerValue],NSLocalizedString(kH, nil)];
...
- (void)saveBtnClick {
    ...
    if ([self.delegate respondsToSelector:@selector(datePickerViewSaveBtnClickDelegate:dateView:)]) {
        [self.delegate datePickerViewSaveBtnClickDelegate:self.selectStr dateView:self];
    }
}
/// 取消按钮点击方法
- (void)cancelBtnClick {
        ...
        case 0: { // 年
            ...
            if (self.dayArr.count > [dayStr integerValue]) {
                if (self.day.integerValue > [dayStr integerValue]) {
                    [pickerView selectRow:[self.dataArray[2] indexOfObject:[dayStr stringByAppendingString:NSLocalizedString(kDay, nil)]] inComponent:2 animated:YES];
                    self.day = [dayStr stringByAppendingString:NSLocalizedString(kDay, nil)];
                }
            }
        } break;
        case 1: { // 月
        ...
        case 2: { // 日
            ...
            if (self.dayArr.count <= [dayStr integerValue]) {
                ...
                if (day_value.integerValue <= [dayStr integerValue]) {
                    ...
                    [pickerView selectRow:[self.dataArray[component] indexOfObject:[dayStr stringByAppendingString:NSLocalizedString(kDay, nil)]] inComponent:component animated:YES];
                }
            }
        } break;
        case 3: { // 时
...
- (NSMutableArray *)yearArr {
    if (!_yearArr) {
        ...
        for (int i = 1970; i < 2099; i ++) {
            [_yearArr addObject:[NSString stringWithFormat:@"%d%@", i,NSLocalizedString(kYear, nil)]];
        }
    }
    return _yearArr;
}
...
- (NSMutableArray *)monthArr {
    ...
    if (!_monthArr) {
        ...
        for (int i = 1; i <= 12; i ++) {
            [_monthArr addObject:[NSString stringWithFormat:@"%d%@", i,NSLocalizedString(kMon, nil)]];
        }
    }
    return _monthArr;
}
...
- (NSMutableArray *)dayArr {
    if (!_dayArr) {
        ...
        for (int i = 1; i <= 31; i ++) {
            [_dayArr addObject:[NSString stringWithFormat:@"%d%@", i,NSLocalizedString(kDay, nil)]];
        }
    }
    return _dayArr;
}
...
- (NSMutableArray *)hourArr {
    if (!_hourArr) {
        ...
        for (int i = 0; i < 24; i ++) {
            [_hourArr addObject:[NSString stringWithFormat:@"%d%@", i,NSLocalizedString(kH, nil)]];
        }
    }
    return _hourArr;
}
...
- (NSMutableArray *)minuteArr {
    if (!_minuteArr) {
        ...
        for (int i = 0; i <= 59; i ++) {
            [_minuteArr addObject:[NSString stringWithFormat:@"%d%@", i,NSLocalizedString(kMin, nil)]];
        }
    }
    return _minuteArr;
}
...
- (void)refreshDay {
    ...
    for (int i = 1; i < [self getDayNumber:self.year.integerValue month:self.month.integerValue].integerValue + 1; i ++) {
        [arr addObject:[NSString stringWithFormat:@"%d%@", i,NSLocalizedString(kDay, nil)]];
    }
